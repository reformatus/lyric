...
Future<Cue> insertNewCue({required String title, required String description}) {
  return db.into(db.cues).insertReturning(
        CuesCompanion(
          id: Value.absent(),
...
Future removeSlideAtIndexFromCue(int index, Cue cue) {
  return (db.update(db.cues)..where((c) => c.uuid.equals(cue.uuid))).write(CuesCompanion(
    content: Value(cue.content.asMap().entries.where((e) => e.key != index).map((e) => e.value).toList()),
  ));
}
